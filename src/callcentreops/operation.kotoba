(ns callcentreops.operation
  "OperationActor -- one coordination request = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (CallCentreAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the CallCentreGovernor (:govern)
  ...
  One graph run = one call-centre-operations coordination request
  ...
  human operator/campaign supervisor. The approver resumes with
...
(defn build
  ...
  `callcentreops.store/Store`).
  opts:
    :advisor      -- a `callcentreops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; CallCentreAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; CallCentreGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
